...
export default function App() {
  ...
  const [filters, setFilters] = useState({ industry: "", contactType: "", leadSource: "", productInterest: "", tag: "", excludeTag: "" });
  ...
  const filtered = useMemo(() => {
    ...
    return contacts.filter((c) => {
      ...
      if (filters.excludeTag) {
        const tagList = (c.tags || "").split(",").map((t) => t.trim()).filter(Boolean);
        if (tagList.includes(filters.excludeTag)) return false;
      }
      return true;
    });
    ...
    <div style={{ background: T.bg, minHeight: "100vh", color: T.ivory, fontFamily: "var(--font-syne), sans-serif" }}>
      ...
      {showFilters && (
        <div style={{ padding: "14px 28px", display: "flex", gap: 12, flexWrap: "wrap", background: T.panel, borderBottom: `1px solid ${T.line}` }}>
          ...
          <FilterSelect label="Exclude tag" value={filters.excludeTag} options={allTags} onChange={(v) => setFilters((f) => ({ ...f, excludeTag: v }))} />
          {activeFilterCount > 0 && <button className="htBtn" style={{ background: "transparent", color: T.rose, alignSelf: "flex-end" }} onClick={() => setFilters({ industry: "", contactType: "", leadSource: "", productInterest: "", tag: "", excludeTag: "" })}>Clear all</button>}
        </div>
      )}
