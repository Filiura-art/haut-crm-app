...
export default function App() {
  ...
  const [filters, setFilters] = useState({ industry: "", contactType: "", leadSource: "", productInterest: "", tags: [], excludeTags: [] });
  ...
  const filtered = useMemo(() => {
    ...
    return contacts.filter((c) => {
      ...
      if (filters.tags.length > 0) {
        ...
        if (!filters.tags.some((t) => tagList.includes(t))) return false;
      }
      if (filters.excludeTags.length > 0) {
        const tagList = (c.tags || "").split(",").map((t) => t.trim()).filter(Boolean);
        if (filters.excludeTags.some((t) => tagList.includes(t))) return false;
      }
      return true;
    });
  ...
  const createTask = useCallback(async () => {
    if (!taskTitle.trim() || !taskDateTime) {
      setTaskError("Fill in both the task text and date/time.");
      return;
    }
    setTaskSaving(true);
    setTaskError("");
    const targets = contacts.filter((c) => selected.has(c.id));
    const leadsPayload = targets.map((c) => ({ name: c.name, email: c.email, phone: c.phone, company: c.company }));

    try {
      const res = await fetch("/api/tasks", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: taskTitle.trim(), dateTime: taskDateTime, leads: leadsPayload, timeZone: Intl.DateTimeFormat().resolvedOptions().timeZone }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);

      const shortCode = Math.random().toString(36).slice(2, 6);
      const dateOnly = taskDateTime.slice(0, 10);
      const taskTag = `Task:${dateOnly}-${shortCode}`;

      for (const c of targets) {
        const existingTags = (c.tags || "").split(",").map((t) => t.trim()).filter(Boolean);
        await saveContact({ ...c, tags: [...existingTags, taskTag].join(", ") });
      }
  ...
  }, [taskTitle, taskDateTime, contacts, selected, saveContact, clearSelection]);

  const bulkExport = useCallback(() => {
    const targets = contacts.filter((c) => selected.has(c.id));
    const cols = ["name","email","phone","company","contactType","country","industry","leadSource","dateReceived","stage","productInterest","occasion","tags","linkedinUrl","linkedinStatus"];
    ...
    <div style={{ background: T.bg, minHeight: "100vh", color: T.ivory, fontFamily: "var(--font-syne), sans-serif" }}>
      ...
      <header style={{ borderBottom: `1px solid ${T.line}`, padding: "22px 28px 18px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <a href="/projects" style={{ fontSize: 12, color: T.brass, textDecoration: "none", border: `1px solid ${T.brass}55`, background: T.brassSoft, padding: "6px 12px", borderRadius: 6 }}>Projects →</a>
      ...
      {showFilters && (
        <div style={{ padding: "14px 28px", display: "flex", gap: 12, flexWrap: "wrap", background: T.panel, borderBottom: `1px solid ${T.line}` }}>
          ...
          <MultiSelectFilter label="Tag" values={filters.tags} options={allTags} onChange={(v) => setFilters((f) => ({ ...f, tags: v }))} />
          <MultiSelectFilter label="Exclude tag" values={filters.excludeTags} options={allTags} onChange={(v) => setFilters((f) => ({ ...f, excludeTags: v }))} />
          {activeFilterCount > 0 && <button className="htBtn" style={{ background: "transparent", color: T.rose, alignSelf: "flex-end" }} onClick={() => setFilters({ industry: "", contactType: "", leadSource: "", productInterest: "", tags: [], excludeTags: [] })}>Clear all</button>}
        </div>
      )}
      ...
      <div style={{ padding: "20px 28px 60px" }}>
        {selected.size > 0 && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", background: T.brassSoft, border: `1px solid ${T.brass}55`, borderRadius: 8, padding: "10px 14px", marginBottom: 14 }}>
            ...
            <button className="htBtn" style={{ background: T.brass, color: T.bg, fontWeight: 600 }} onClick={() => setShowTaskModal(true)}><Plus size={14} /> Create task</button>
      ...
      {showTaskModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 16 }} onClick={() => setShowTaskModal(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: T.panel, border: `1px solid ${T.line}`, borderRadius: 10, width: "100%", maxWidth: 440, padding: 24 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h2 style={{ fontFamily: "var(--font-syne), sans-serif", fontWeight: 700, fontSize: 18, margin: 0 }}>New task — {selected.size} lead(s)</h2>
              <button className="htBtn" style={{ background: "transparent", color: T.ivoryFaint, padding: 4 }} onClick={() => setShowTaskModal(false)}><X size={18} /></button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <Field label="Task text"><input className="htInput" value={taskTitle} onChange={(e) => setTaskTitle(e.target.value)} placeholder="e.g. Follow up on pricing" /></Field>
              <Field label="Date & time"><input type="datetime-local" className="htInput" value={taskDateTime} onChange={(e) => setTaskDateTime(e.target.value)} /></Field>
              {taskError && <div style={{ color: T.rose, fontSize: 12 }}>{taskError}</div>}
            </div>
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 20 }}>
              <button className="htBtn" style={{ background: T.panelAlt, color: T.ivory, border: `1px solid ${T.line}` }} onClick={() => setShowTaskModal(false)}>Cancel</button>
              <button className="htBtn" style={{ background: T.brass, color: T.bg, fontWeight: 600 }} onClick={createTask} disabled={taskSaving}>{taskSaving ? "Creating…" : "Add to Calendar"}</button>
            </div>
          </div>
        </div>
      )}
...
function MultiSelectFilter({ label, values, options, onChange }) {
  ...
  return (
    <div style={{ minWidth: 160, position: "relative" }} ref={ref}>
      <div style={{ fontSize: 11, color: T.ivoryFaint, marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.06em" }}>{label}</div>
      <button
        type="button"
        className="htInput"
      ...
      >
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {values.length === 0 ? "All" : values.length === 1 ? values[0] : `${values.length} selected`}
        </span>
        <ChevronDown size={13} />
      </button>
      {open && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, marginTop: 4, background: T.panelAlt, border: `1px solid ${T.line}`, borderRadius: 6, maxHeight: 220, overflowY: "auto", zIndex: 20 }}>
          {options.length === 0 && <div style={{ padding: "8px 10px", fontSize: 12, color: T.ivoryFaint }}>No tags yet</div>}
          {options.map((opt) => (
            <label key={opt} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", fontSize: 13, cursor: "pointer" }}>
              <input type="checkbox" checked={values.includes(opt)} onChange={() => toggle(opt)} style={{ accentColor: T.brass }} />
              {opt}
            </label>
          ))}
        </div>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
        <thead>
          <tr style={{ background: T.panel, textAlign: "left" }}>
            ...
            {["Name","Company","Type","Industry","Source","Stage","Tags","LinkedIn",""].map((h) => (
