...
export async function createTaskEvent({ title, dateTime, leads, timeZone }) {
  ...
  // <input type="datetime-local">. Google Calendar's API requires full RFC3339
  // with seconds, so we normalize that here before sending.
  const normalizedDateTime = dateTime.length === 16 ? `${dateTime}:00` : dateTime;
  const [datePart, timePart] = normalizedDateTime.split("T");
  const [h, m] = timePart.split(":").map(Number);
  const totalMinutes = h * 60 + m + 30;
  const endHour = Math.floor(totalMinutes / 60) % 24;
  const endMinute = totalMinutes % 60;
  const endDateTime = `${datePart}T${String(endHour).padStart(2, "0")}:${String(endMinute).padStart(2, "0")}:00`;
  ...
  const res = await calendar.events.insert({
    ...
    requestBody: {
      ...
      description,
      start: { dateTime: normalizedDateTime, timeZone: tz },
